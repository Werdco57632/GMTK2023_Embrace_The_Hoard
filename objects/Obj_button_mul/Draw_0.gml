...
locked = 0
if (Obj_cursor.point_multiplier <= 0) {locked = 1}
if (Obj_cursor.point_multiplier >= 10) {locked = 2}
if (collision_rectangle(x-48,y,x-32,y+16,Obj_cursor,false,true)) {
	...
	if (mouse_check_button_released(mb_left)) {
		if (locked != 1) {
			if (Obj_cursor.point_multiplier <= 1) {Obj_cursor.point_multiplier -= 0.1}
			else {Obj_cursor.point_multiplier -= 1}
		}
	}
...
draw_text(x-30,y+5,"points:x"+string(Obj_cursor.point_multiplier))